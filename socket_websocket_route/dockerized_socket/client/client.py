...
# websocket connect and send function
...
def E0action(content, aesagent):
    ...
    print("obu_info : ",list(decrypted_data[0:8])), print("issue_info : ",list(decrypted_data[8:]))
    re_enc_obu = str(aesagent.re_encrypt(decrypted_data[0:8]))
    re_enc_issue = str(aesagent.re_encrypt(decrypted_data[8:]))
    
    # format as json string for websocket
    formatted_data = '{"sender":"01","destination":"ypass-service-api","contents":{"obu-info":"'+re_enc_obu+'","issue-info":"'+re_enc_issue+'"}}'

    try:
        asyncio.get_event_loop().run_until_complete(websocketSendData(formatted_data))
    except:
